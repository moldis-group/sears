...
def get_s_bridge_ring_size(mol, s_idx, o_idx, c_idx):
    """
    Find ring(s) containing S, peroxy O, and donor C.
    ...
    for ring in ring_info.AtomRings():
        ring_set = set(ring)
        if s_idx in ring_set and o_idx in ring_set and c_idx in ring_set:
...
def make_s_bridge_template_with_maps(mol):
    """
    Build the S-bridge template from the QOOH radical.

    Steps:
      - add explicit Hs
      - convert OOH hydrogen -> S
      - connect S to donor radical carbon
      - keep all existing stereochemistry fixed
      - mark donor carbon, S, and peroxy O with atom-map numbers

    Returns:
      (mol_noH, donor_mapnum, s_mapnum, o_mapnum, ring_size)
    or
    ...
    # Convert hydroperoxide H into S
    s_atom = rw.GetAtomWithIdx(h_idx)
    s_atom.SetAtomicNum(16)
    s_atom.SetFormalCharge(0)
    s_atom.SetNumExplicitHs(0)
    s_atom.SetNoImplicit(True)

    # Ensure S-O and S-C bonds exist
    ...
    m_newH.GetAtomWithIdx(h_idx).SetAtomMapNum(s_mapnum)
    m_newH.GetAtomWithIdx(o_idx).SetAtomMapNum(o_mapnum)

    ring_size = get_s_bridge_ring_size(m_newH, h_idx, o_idx, c_idx)
    ...
    return m_new, donor_mapnum, s_mapnum, o_mapnum, ring_size
...
def sbridge_to_qooh(molS):
    """
    Convert S-bridge scaffold back to QOOH by:
      - changing S -> H
      - removing S-C bond
    ...
    s_idxs = [a.GetIdx() for a in rw.GetAtoms() if a.GetSymbol() == "S"]
    if len(s_idxs) != 1:
        return None
    s_idx = s_idxs[0]

    s_atom = rw.GetAtomWithIdx(s_idx)
    nbrs = [n.GetIdx() for n in s_atom.GetNeighbors()]
    ...
    bond_sc = rw.GetBondBetweenAtoms(s_idx, c_idx)
    if bond_sc is None:
        return None
    rw.RemoveBond(s_idx, c_idx)

    s_atom = rw.GetAtomWithIdx(s_idx)
    s_atom.SetAtomicNum(1)
    s_atom.SetFormalCharge(0)
    s_atom.SetNumExplicitHs(0)
    s_atom.SetNoImplicit(True)
...
def sbridge_to_roo(molS):
    """
    Convert S-bridge scaffold to ROO radical by:
      - removing the S atom entirely
      - removing both S-O and S-C bonds
    ...
    # Remove S atom after noting donor carbon
    rw.RemoveAtom(s_idx)
...
def endpoints_match(iso1, iso2):
    ...
    """
    q1 = sbridge_to_qooh(iso1)
    q2 = sbridge_to_qooh(iso2)
    if q1 is None or q2 is None:
        return False, None, None, None, None

    r1 = sbridge_to_roo(iso1)
    r2 = sbridge_to_roo(iso2)
...
with open(input_file) as infile, open(output_file, "w") as out:
    ...
    for line in infile:
        ...
        base_S, donor_mapnum, s_mapnum, o_mapnum, ring_size = make_s_bridge_template_with_maps(mol)
        if base_S is None:
            n_skipped += 1
            continue

        donor_idx = get_atom_idx_by_mapnum(base_S, donor_mapnum)
        s_idx = get_atom_idx_by_mapnum(base_S, s_mapnum)

        if donor_idx is None or s_idx is None:
            n_skipped += 1
            continue

        donor_atom = base_S.GetAtomWithIdx(donor_idx)
        ...
        iso1 = clone_with_donor_chirality(
            base_S, donor_idx, rdchem.ChiralType.CHI_TETRAHEDRAL_CW
        )
        iso2 = clone_with_donor_chirality(
            base_S, donor_idx, rdchem.ChiralType.CHI_TETRAHEDRAL_CCW
        )
        ...
        # Collapse check: not actually two unique S-bridge stereomers
        if smi1 == smi2:
            n_not_exact2 += 1
        ...
        for i, smi_out in enumerate(smis, start=1):
            new_id = f"{mol_id}_Sbridge_{i:02d}"
            out.write(f"{smi_out},{new_id},{ring_size}-mem\n")
            if ring_size is not None:
                print(f"{new_id}: new S-ring is {ring_size}-membered")
            else:
                print(f"{new_id}: could not determine S-ring size")
            n_written += 1

print("Total QOOH entries read:", n_total)
print("Exact two S-bridge stereomers written:", n_written)
